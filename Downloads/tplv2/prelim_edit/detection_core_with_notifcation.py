...
# Load YAML Basic configuration
yaml_config = load_yaml_config()

# Load notification yaml 
...
# Notification Logic requirements
# Mail configs
                            ...
                            if person_conf >= ROI_PERSON_CONFIDENCE[i]:
                                ...
                                if self.roi_state[i] == "IDLE":
                                    ...
                                    if os.path.exists(save_path):  
                                        ...
                                        threading.Thread(
                                            target=email_coke,
                                            args=(html_string, smtp_server, email_ids, dtTimestamp1.strftime("%d-%b-%y"),save_path),
                                            daemon=True
                                        ).start()
                                    
                                    threading.Thread(
                                        target=send_sms,
                                        args=(message, mobile_numbers),
                                        daemon=True 
                                    ).start() 
